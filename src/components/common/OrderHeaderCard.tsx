...
import { User, Calendar, Receipt, Info, ArrowUpRight, ArrowDownLeft, Clock } from "lucide-react";
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex flex-col gap-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight sm:text-3xl">
            {partner?.name || (isPurchase ? "General Supplier" : "Walk-in Customer")}
          </h1>
          {partner?.phone && (
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-2xl border bg-card p-4 shadow-sm">
          <div className="flex items-center gap-3 mb-2 text-muted-foreground">
            ...
            <span className="text-xs font-semibold uppercase tracking-wide">Paid Amount</span>
          </div>
        ...
        <div className="rounded-2xl border bg-card p-4 shadow-sm">
          <div className="flex items-center gap-3 mb-2 text-muted-foreground">
            ...
            <span className="text-xs font-semibold uppercase tracking-wide">Loan / Pending</span>
          </div>
          <p className="text-xl font-bold text-rose-600">{formatCurrency(loanAmount)}</p>
        </div>

        <div className={cn(
          "rounded-2xl border p-4 shadow-sm",
          balance <= 0 ? "bg-emerald-50/50 border-emerald-100" : "bg-amber-50/50 border-amber-100"
        )}>
          <div className="flex items-center gap-3 mb-2 text-muted-foreground">
            <div className={cn(
              "rounded-full p-1.5",
              balance <= 0 ? "bg-emerald-100 text-emerald-600" : "bg-amber-100 text-amber-600"
            )}>
      ...
      {description && (
        <div className="rounded-2xl border bg-muted/30 p-4">
          <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider block mb-2">Note</span>
